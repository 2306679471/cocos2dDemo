...
PowerBuyinfo* PowerBuyinfo::create(int idx)
{
	PowerBuyinfo* power = new PowerBuyinfo;
	if (power&&power->init(idx))
	{
		power->autorelease();
	}
	else{
...
bool PowerBuyinfo::init(int idx)
{
	if (!Node::init())
	{
		return false;
		...
		this->addChild(info);
		SceneBuy* buy = SceneBuy::create(1,idx);
		this->addChild(buy);
		//±³¾°
