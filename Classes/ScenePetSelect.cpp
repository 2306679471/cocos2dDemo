...
ScenePetSelect* ScenePetSelect::create()
{
	ScenePetSelect* pet = new ScenePetSelect;
	if (pet&&pet->init())
	{
		pet->autorelease();
	}
...
bool ScenePetSelect::init()
{
	...
	else
	{
		...
		SelectLayerinfo* sinfo = SelectLayerinfo::create("selectpet_tips.png");
		...
		for (int i = 0; i < L; i++)
		{
			...
			//高光精灵背景
			Sprite* sphl = Sprite::create("select_iconback1.png");
			sp.push_back(sphl);
			...
			//添加英雄图片
			std::sprintf(filename, "selectpet_peticon%d.png", i);
...
const char* ScenePetSelect::gb23122utf8(const char* gb2312)
{
